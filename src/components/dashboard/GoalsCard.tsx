    ...
    <Card title="Goals">
      <div className="mt-2">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="text-body font-semibold">${goal.targetAmount.toLocaleString()}</div>
        </div>
        <div className="flex items-end gap-2 mb-4">
          <span className="text-card-currency">$</span>
          <span className="text-card-value">{goal.currentAmount.toLocaleString()}</span>
        </div>
        ...
        <div className="flex items-center gap-2 mt-4 text-sm" style={{ color: 'var(--accent-purple)' }}>
          ...
          <span>Save ${(goal.targetAmount - goal.currentAmount).toLocaleString()} more to reach your goal!</span>
        </div>
      </div>
        ...
        {goals.map((_, idx) => (
          <button
            ...
            className={`w-2 h-2 rounded-full transition-all ${
              idx === activeIndex ? 'w-8 bg-accent-purple' : 'bg-text-secondary'
            }`}
